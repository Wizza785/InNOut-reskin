        ...
        Image(uiImage: #imageLiteral(resourceName: "loginbkgd.png"))
            ...
            .overlay(VStack(spacing: 9) {
                ...
                Button(action: self.login) {
                    Text("Sign In")
                }.foregroundColor(.white).padding().background(Rectangle().cornerRadius(10).foregroundColor(.red)).padding(20).offset(x: 0, y: -260)
            }
            .background(Group {
                EmptyView()
            }, alignment: .center)
   ...
   */
    ...
    func login(){
       ...
       // let cookiesArray = HTTPCookieStorage.shared.cookies
        //var cookiesStore =  URLSession.shared.configuration.httpCookieStorage?.cookies
        ...
        //print(cookiesArray ?? "nil")
          semaphore.signal()
        }
        var cookies = readCookie(forURL:)
        cookies = readCookie
        print(cookies)
